...
namespace Systems.MineSystem.EnemySystem.Mob.RattleSnake.View
{
    public sealed class SnakeView : MonoBehaviour, IDamageable
    {
        ...
        public bool ValidateReferences()
        {
            if (body != null && terrainCollider != null &&
                ...
                return true;
            Debug.LogError(
                "SnakeView requires a Rigidbody2D, non-trigger terrain collider, " +
                "trigger hurtbox, SpriteRenderer, and EnemyAnimationController.",
                this);
            return false;
        }

        public void ApplyConfig(SnakeConfigScriptable config)
        {
            animationController.ApplyProfile(config.AnimationProfile);
            spriteRenderer.color = config.SnakeColor;
