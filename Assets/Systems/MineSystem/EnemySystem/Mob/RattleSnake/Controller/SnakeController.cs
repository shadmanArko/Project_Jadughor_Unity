...
namespace Systems.MineSystem.EnemySystem.Mob.RattleSnake.Controller
{
    public sealed class SnakeController : IEnemyController
    {
            ...
            IEnemyChaseTargetResolver chaseResolver)
        {
            ...
            _stateMachine = new SnakeStateMachine(
                _model,
                view,
                target,
                attack,
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(SnakeController));
            if (IsActive)
                Release();
            if (initializeData.Config is not SnakeConfigScriptable config)
            {
                throw new ArgumentException(
                    "SnakeController requires SnakeConfigScriptable.");
            }
            if (!_view.ValidateReferences())
                throw new InvalidOperationException("SnakeView is not configured.");
                    ...
                    initializeData.SpawnWorldPosition))
            {
                throw new InvalidOperationException(
                    $"Snake cannot fit at spawn cell {initializeData.SpawnGridPosition}.");
            }
